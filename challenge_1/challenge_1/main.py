...
def run_training(
    net_name: str, epochs: int, cloud_run: bool, fine_tune: bool, cloud_tuner: bool, test: bool
) -> None:
    """Run the training."""
    log.setup(log_level=logging.DEBUG)

    if cloud_run:
        train_on_gcp(net_name=net_name, epochs=epochs, fine_tune=fine_tune, cloud_tuner=cloud_tuner)
    else:
